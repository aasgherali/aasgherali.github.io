---
layout: page
title: Calculus II
description: Teaching Assistant · Fall 2022-23 · Lahore University of Management Sciences
importance: 8
category: Lahore University of Management Sciences
---

Served as Teaching Assistant for the second semester calculus course, covering multivariable calculus and series. Conducted weekly problem sessions and held office hours.

**Role:** Teaching Assistant

**Offered:** Fall 2022–23

---

#### Topics Covered

- Sequences and series; convergence tests
- Power series and Taylor/Maclaurin expansions
- Vectors in 2D and 3D; dot and cross products
- Multivariable functions, partial derivatives, gradient
- Optimization of functions of several variables
- Double and triple integrals
- Introduction to vector calculus (Green's theorem, Stokes' theorem)
